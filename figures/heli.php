  <?php
        ...
        $productId = 458; // 🔁 Change this per product page
        ...
        $initialStock = $variants[132]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/heli.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/heli.jpg" checked hidden>
                                    <img src="img/products/heli.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/heli2.jpg" hidden>
                                    <img src="img/products/heli2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/heli3.jpg" hidden>
                                    <img src="img/products/heli3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 132, 133, and 134
                                            $stock132 = $variants[132]['stock_quantity'] ?? 0;
                                            $is132Disabled = ($stock132 <= 0);

                                            $stock133 = $variants[133]['stock_quantity'] ?? 0;
                                            $is133Disabled = ($stock133 <= 0);

                                            $stock134 = $variants[134]['stock_quantity'] ?? 0;
                                            $is134Disabled = ($stock134 <= 0);

                                            // 2. Logic: Auto-select the first available variant on load
                                            $defaultActiveId = null;
                                            if (!$is132Disabled) {
                                                $defaultActiveId = 132;
                                            } elseif (!$is133Disabled) {
                                                $defaultActiveId = 133;
                                            } elseif (!$is134Disabled) {
                                                $defaultActiveId = 134;
                                            }

                                            // 3. Set initial values for display based on the selected variant
                                            if ($defaultActiveId == 132) {
                                                $initialStock = $stock132;
                                                $initialPrice = "299";
                                            } elseif ($defaultActiveId == 133) {
                                                $initialStock = $stock133;
                                                $initialPrice = "239";
                                            } elseif ($defaultActiveId == 134) {
                                                $initialStock = $stock134;
                                                $initialPrice = "429";
                                            } else {
                                                $initialStock = 0;
                                                $initialPrice = "299"; // Default fallback
                                            }

                                            $allOutOfStock = ($is132Disabled && $is133Disabled && $is134Disabled);
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 132) ? 'active' : ''; ?> <?php echo $is132Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="299" 
                                                data-variant-id="132" 
                                                data-stock="<?php echo $stock132; ?>"
                                                <?php echo $is132Disabled ? 'disabled' : ''; ?>>
                                            Red <?php echo $is132Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 133) ? 'active' : ''; ?> <?php echo $is133Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="239" 
                                                data-variant-id="133" 
                                                data-stock="<?php echo $stock133; ?>"
                                                <?php echo $is133Disabled ? 'disabled' : ''; ?>>
                                            Light Blue <?php echo $is133Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 134) ? 'active' : ''; ?> <?php echo $is134Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="429" 
                                                data-variant-id="134" 
                                                data-stock="<?php echo $stock134; ?>"
                                                <?php echo $is134Disabled ? 'disabled' : ''; ?>>
                                            Green <?php echo $is134Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>
