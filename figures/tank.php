  <?php
        ...
        $productId = 455; // 🔁 Change this per product page
        ...
        $initialStock = $variants[124]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/tank.png" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/tank.png" checked hidden>
                                    <img src="img/products/tank.png" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/tank2.jpg" hidden>
                                    <img src="img/products/tank2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/tank3.jpg" hidden>
                                    <img src="img/products/tank3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 124, 125, and 126
                                            $stock124 = $variants[124]['stock_quantity'] ?? 0;
                                            $is124Disabled = ($stock124 <= 0);

                                            $stock125 = $variants[125]['stock_quantity'] ?? 0;
                                            $is125Disabled = ($stock125 <= 0);

                                            $stock126 = $variants[126]['stock_quantity'] ?? 0;
                                            $is126Disabled = ($stock126 <= 0);

                                            // 2. Logic: Auto-select the first available color on load
                                            $defaultActiveId = null;
                                            if (!$is124Disabled) {
                                                $defaultActiveId = 124;
                                            } elseif (!$is125Disabled) {
                                                $defaultActiveId = 125;
                                            } elseif (!$is126Disabled) {
                                                $defaultActiveId = 126;
                                            }

                                            // 3. Set initial values for display based on the selected variant
                                            if ($defaultActiveId == 124) {
                                                $initialStock = $stock124;
                                                $initialPrice = "2199";
                                            } elseif ($defaultActiveId == 125) {
                                                $initialStock = $stock125;
                                                $initialPrice = "2399";
                                            } elseif ($defaultActiveId == 126) {
                                                $initialStock = $stock126;
                                                $initialPrice = "2349";
                                            } else {
                                                $initialStock = 0;
                                                $initialPrice = "2199"; // Fallback price
                                            }

                                            $allOutOfStock = ($is124Disabled && $is125Disabled && $is126Disabled);
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 124) ? 'active' : ''; ?> <?php echo $is124Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="2199" 
                                                data-variant-id="124" 
                                                data-stock="<?php echo $stock124; ?>"
                                                <?php echo $is124Disabled ? 'disabled' : ''; ?>>
                                            Leopard Tank <?php echo $is124Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 125) ? 'active' : ''; ?> <?php echo $is125Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="2399" 
                                                data-variant-id="125" 
                                                data-stock="<?php echo $stock125; ?>"
                                                <?php echo $is125Disabled ? 'disabled' : ''; ?>>
                                            Tiger Tank <?php echo $is125Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 126) ? 'active' : ''; ?> <?php echo $is126Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="2349" 
                                                data-variant-id="126" 
                                                data-stock="<?php echo $stock126; ?>"
                                                <?php echo $is126Disabled ? 'disabled' : ''; ?>>
                                            Crawler Tank <?php echo $is126Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>             
