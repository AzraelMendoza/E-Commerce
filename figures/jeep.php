  <?php
        ...
        $productId = 452; // 🔁 Change this per product page
        ...
        $initialStock = $variants[118]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/jeep.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/jeep.jpg" checked hidden>
                                    <img src="img/products/jeep.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/jeep2.jpg" hidden>
                                    <img src="img/products/jeep2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/jeep3.jpg" hidden>
                                    <img src="img/products/jeep3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 118, 119, and 120
                                            $stock118 = $variants[118]['stock_quantity'] ?? 0;
                                            $is118Disabled = ($stock118 <= 0);

                                            $stock119 = $variants[119]['stock_quantity'] ?? 0;
                                            $is119Disabled = ($stock119 <= 0);

                                            $stock120 = $variants[120]['stock_quantity'] ?? 0;
                                            $is120Disabled = ($stock120 <= 0);

                                            // 2. Logic: Auto-select the first available color on load
                                            $defaultActiveId = null;
                                            if (!$is118Disabled) {
                                                $defaultActiveId = 118;
                                            } elseif (!$is119Disabled) {
                                                $defaultActiveId = 119;
                                            } elseif (!$is120Disabled) {
                                                $defaultActiveId = 120;
                                            }

                                            // 3. Set initial values for display based on the auto-selected variant
                                            if ($defaultActiveId == 118) {
                                                $initialStock = $stock118;
                                                $initialPrice = "1199";
                                            } elseif ($defaultActiveId == 119) {
                                                $initialStock = $stock119;
                                                $initialPrice = "1219";
                                            } elseif ($defaultActiveId == 120) {
                                                $initialStock = $stock120;
                                                $initialPrice = "1209";
                                            } else {
                                                $initialStock = 0;
                                                $initialPrice = "1199"; // Fallback
                                            }

                                            $allOutOfStock = ($is118Disabled && $is119Disabled && $is120Disabled);
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 118) ? 'active' : ''; ?> <?php echo $is118Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="1199" 
                                                data-variant-id="118" 
                                                data-stock="<?php echo $stock118; ?>"
                                                <?php echo $is118Disabled ? 'disabled' : ''; ?>>
                                            Gray <?php echo $is118Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 119) ? 'active' : ''; ?> <?php echo $is119Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="1219" 
                                                data-variant-id="119" 
                                                data-stock="<?php echo $stock119; ?>"
                                                <?php echo $is119Disabled ? 'disabled' : ''; ?>>
                                            Yellow <?php echo $is119Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 120) ? 'active' : ''; ?> <?php echo $is120Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="1209" 
                                                data-variant-id="120" 
                                                data-stock="<?php echo $stock120; ?>"
                                                <?php echo $is120Disabled ? 'disabled' : ''; ?>>
                                            Red <?php echo $is120Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>
