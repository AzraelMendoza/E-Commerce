  <?php
        ...
        $productId = 451; // 🔁 Change this per product page
        ...
        $initialStock = $variants[116]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/f1.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/f1.jpg" checked hidden>
                                    <img src="img/products/f1.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/f1r.jpg" hidden>
                                    <img src="img/products/f1r.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/f1b2.jpg" hidden>
                                    <img src="img/products/f1b2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 116 and 117
                                            $stock116 = $variants[116]['stock_quantity'] ?? 0;
                                            $is116Disabled = ($stock116 <= 0);

                                            $stock117 = $variants[117]['stock_quantity'] ?? 0;
                                            $is117Disabled = ($stock117 <= 0);

                                            // 2. Logic: Auto-select the first available color on load
                                            $defaultActiveId = null;
                                            if (!$is116Disabled) {
                                                $defaultActiveId = 116;
                                            } elseif (!$is117Disabled) {
                                                $defaultActiveId = 117;
                                            }

                                            // 3. Set initial values for display
                                            if ($defaultActiveId == 116) {
                                                $initialStock = $stock116;
                                            } elseif ($defaultActiveId == 117) {
                                                $initialStock = $stock117;
                                            } else {
                                                $initialStock = 0;
                                            }

                                            $allOutOfStock = ($is116Disabled && $is117Disabled);
                                            $flatPrice = "1249";
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 116) ? 'active' : ''; ?> <?php echo $is116Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="<?php echo $flatPrice; ?>" 
                                                data-variant-id="116" 
                                                data-stock="<?php echo $stock116; ?>"
                                                <?php echo $is116Disabled ? 'disabled' : ''; ?>>
                                            Black <?php echo $is116Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 117) ? 'active' : ''; ?> <?php echo $is117Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="<?php echo $flatPrice; ?>" 
                                                data-variant-id="117" 
                                                data-stock="<?php echo $stock117; ?>"
                                                <?php echo $is117Disabled ? 'disabled' : ''; ?>>
                                            Red <?php echo $is117Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>
