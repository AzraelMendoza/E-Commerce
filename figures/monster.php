  <?php
        ...
        $productId = 448; // 🔁 Change this per product page
        ...
        $initialStock = $variants[111]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/monster.PNG" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/monster.PNG" checked hidden>
                                    <img src="img/products/monster.PNG" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/monster2.jpg" hidden>
                                    <img src="img/products/monster2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/monster3.jpg" hidden>
                                    <img src="img/products/monster3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                        ...
                                        <div class="d-flex gap-2">
                                            <?php 
                                                // 1. Fetch stock for variants 111, 112, and 113
                                                $stock111 = $variants[111]['stock_quantity'] ?? 0;
                                                $is111Disabled = ($stock111 <= 0);

                                                $stock112 = $variants[112]['stock_quantity'] ?? 0;
                                                $is112Disabled = ($stock112 <= 0);

                                                $stock113 = $variants[113]['stock_quantity'] ?? 0;
                                                $is113Disabled = ($stock113 <= 0);

                                                // 2. Logic: Auto-select the first available color on load
                                                $defaultActiveId = null;
                                                if (!$is111Disabled) {
                                                    $defaultActiveId = 111;
                                                } elseif (!$is112Disabled) {
                                                    $defaultActiveId = 112;
                                                } elseif (!$is113Disabled) {
                                                    $defaultActiveId = 113;
                                                }

                                                // 3. Set initial stock value based on selection
                                                if ($defaultActiveId == 111) $initialStock = $stock111;
                                                elseif ($defaultActiveId == 112) $initialStock = $stock112;
                                                elseif ($defaultActiveId == 113) $initialStock = $stock113;
                                                else $initialStock = 0;

                                                $allOutOfStock = ($is111Disabled && $is112Disabled && $is113Disabled);
                                                $flatPrice = "1299";
                                            ?>

                                            <button type="button" 
                                                    class="btn option-btn <?php echo ($defaultActiveId == 111) ? 'active' : ''; ?> <?php echo $is111Disabled ? 'disabled-variant' : ''; ?>" 
                                                    data-price="<?php echo $flatPrice; ?>" 
                                                    data-variant-id="111" 
                                                    data-stock="<?php echo $stock111; ?>"
                                                    <?php echo $is111Disabled ? 'disabled' : ''; ?>>
                                                Orange <?php echo $is111Disabled ? '(Sold Out)' : ''; ?>
                                            </button>

                                            <button type="button" 
                                                    class="btn option-btn <?php echo ($defaultActiveId == 112) ? 'active' : ''; ?> <?php echo $is112Disabled ? 'disabled-variant' : ''; ?>" 
                                                    data-price="<?php echo $flatPrice; ?>" 
                                                    data-variant-id="112" 
                                                    data-stock="<?php echo $stock112; ?>"
                                                    <?php echo $is112Disabled ? 'disabled' : ''; ?>>
                                                Sky Blue <?php echo $is112Disabled ? '(Sold Out)' : ''; ?>
                                            </button>

                                            <button type="button" 
                                                    class="btn option-btn <?php echo ($defaultActiveId == 113) ? 'active' : ''; ?> <?php echo $is113Disabled ? 'disabled-variant' : ''; ?>" 
                                                    data-price="<?php echo $flatPrice; ?>" 
                                                    data-variant-id="113" 
                                                    data-stock="<?php echo $stock113; ?>"
                                                    <?php echo $is113Disabled ? 'disabled' : ''; ?>>
                                                Green <?php echo $is113Disabled ? '(Sold Out)' : ''; ?>
                                            </button>
                                        </div>
                                </div>
