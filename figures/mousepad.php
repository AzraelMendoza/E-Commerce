<?php
        ...
        $productId = 425; // 🔁 Change this per product page
        ...
        $initialStock = $variants[59]['stock_quantity'] ?? 30;
...
<head>
    ...
    <title>Mousepad Chessboard</title>
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/mousepad1.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mousepad1.jpg" checked hidden>
                                    <img src="img/products/mousepad1.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mousepad2.jpg" hidden>
                                    <img src="img/products/mousepad2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mousepad3.jpg" hidden>
                                    <img src="img/products/mousepad3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                        ...
                        <!-- Product Variants -->
                            <div class="mt-4">
                                ...
                                <div class="d-flex gap-2">
                                    <?php 
                                        // 1. Fetch stock for variants 59, 60, and 61
                                        $stock59 = $variants[59]['stock_quantity'] ?? 0;
                                        $is59Disabled = ($stock59 <= 0);

                                        $stock60 = $variants[60]['stock_quantity'] ?? 0;
                                        $is60Disabled = ($stock60 <= 0);

                                        $stock61 = $variants[61]['stock_quantity'] ?? 0;
                                        $is61Disabled = ($stock61 <= 0);

                                        // 2. Logic: Auto-select the first available color on load
                                        $defaultActiveId = null;
                                        $initialStock = 0;

                                        if (!$is59Disabled) {
                                            $defaultActiveId = 59;
                                            $initialStock = $stock59;
                                        } elseif (!$is60Disabled) {
                                            $defaultActiveId = 60;
                                            $initialStock = $stock60;
                                        } elseif (!$is61Disabled) {
                                            $defaultActiveId = 61;
                                            $initialStock = $stock61;
                                        }

                                        // Check if every single option is out of stock
                                        $allOutOfStock = ($is59Disabled && $is60Disabled && $is61Disabled);
                                    ?>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 59) ? 'active' : ''; ?> <?php echo $is59Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="299" 
                                            data-variant-id="59" 
                                            data-stock="<?php echo $stock59; ?>"
                                            <?php echo $is59Disabled ? 'disabled' : ''; ?>>
                                        Black <?php echo $is59Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 60) ? 'active' : ''; ?> <?php echo $is60Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="339" 
                                            data-variant-id="60" 
                                            data-stock="<?php echo $stock60; ?>"
                                            <?php echo $is60Disabled ? 'disabled' : ''; ?>>
                                        Maple <?php echo $is60Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 61) ? 'active' : ''; ?> <?php echo $is61Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="319" 
                                            data-variant-id="61" 
                                            data-stock="<?php echo $stock61; ?>"
                                            <?php echo $is61Disabled ? 'disabled' : ''; ?>>
                                        Rosewood <?php echo $is61Disabled ? '(Sold Out)' : ''; ?>
                                    </button>
                                </div>
                            </div>
                            ...
                            <form id="cartForm" action="buynow.php" method="POST" style="display:none;">
                                ...
                                <input type="hidden" id="variant_id" name="variant_id" value="<?php echo $defaultActiveId ?: 59; ?>">
