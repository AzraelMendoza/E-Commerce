  <?php
        ...
        $productId = 453; // 🔁 Change this per product page
        ...
        $initialStock = $variants[121]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/rob1.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/rob1.jpg" checked hidden>
                                    <img src="img/products/rob1.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/rob2.jpg" hidden>
                                    <img src="img/products/rob2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/rob3.jpg" hidden>
                                    <img src="img/products/rob3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-7 ps-lg-5">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="d-flex gap-2">
                                    <?php 
                                        // 1. Fetch stock for variants 121, 122, and 123
                                        $stock121 = $variants[121]['stock_quantity'] ?? 0;
                                        $is121Disabled = ($stock121 <= 0);

                                        $stock122 = $variants[122]['stock_quantity'] ?? 0;
                                        $is122Disabled = ($stock122 <= 0);

                                        $stock123 = $variants[123]['stock_quantity'] ?? 0;
                                        $is123Disabled = ($stock123 <= 0);

                                        // 2. Logic: Auto-select the first available color on load
                                        $defaultActiveId = null;
                                        if (!$is121Disabled) {
                                            $defaultActiveId = 121;
                                        } elseif (!$is122Disabled) {
                                            $defaultActiveId = 122;
                                        } elseif (!$is123Disabled) {
                                            $defaultActiveId = 123;
                                        }

                                        // 3. Set initial values for display
                                        if ($defaultActiveId == 121) $initialStock = $stock121;
                                        elseif ($defaultActiveId == 122) $initialStock = $stock122;
                                        elseif ($defaultActiveId == 123) $initialStock = $stock123;
                                        else $initialStock = 0;

                                        $allOutOfStock = ($is121Disabled && $is122Disabled && $is123Disabled);
                                        $flatPrice = "899";
                                    ?>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 121) ? 'active' : ''; ?> <?php echo $is121Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="<?php echo $flatPrice; ?>" 
                                            data-variant-id="121" 
                                            data-stock="<?php echo $stock121; ?>"
                                            <?php echo $is121Disabled ? 'disabled' : ''; ?>>
                                        Gold <?php echo $is121Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 122) ? 'active' : ''; ?> <?php echo $is122Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="<?php echo $flatPrice; ?>" 
                                            data-variant-id="122" 
                                            data-stock="<?php echo $stock122; ?>"
                                            <?php echo $is122Disabled ? 'disabled' : ''; ?>>
                                        Red <?php echo $is122Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 123) ? 'active' : ''; ?> <?php echo $is123Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="<?php echo $flatPrice; ?>" 
                                            data-variant-id="123" 
                                            data-stock="<?php echo $stock123; ?>"
                                            <?php echo $is123Disabled ? 'disabled' : ''; ?>>
                                        White Blue <?php echo $is123Disabled ? '(Sold Out)' : ''; ?>
                                    </button>
                                </div>
                            </div>
