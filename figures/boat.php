  <?php
        ...
        $productId = 459; // 🔁 Change this per product page
        ...
        $initialStock = $variants[135]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/speedboat3.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/speedboat3.jpg" checked hidden>
                                    <img src="img/products/speedboat3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/speedboat2.jpg" hidden>
                                    <img src="img/products/speedboat2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/speedboat.jpg" hidden>
                                    <img src="img/products/speedboat.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 135, 136, and 137
                                            $stock135 = $variants[135]['stock_quantity'] ?? 0;
                                            $is135Disabled = ($stock135 <= 0);

                                            $stock136 = $variants[136]['stock_quantity'] ?? 0;
                                            $is136Disabled = ($stock136 <= 0);

                                            $stock137 = $variants[137]['stock_quantity'] ?? 0;
                                            $is137Disabled = ($stock137 <= 0);

                                            // 2. Logic: Auto-select the first available variant on load
                                            $defaultActiveId = null;
                                            if (!$is135Disabled) {
                                                $defaultActiveId = 135;
                                            } elseif (!$is136Disabled) {
                                                $defaultActiveId = 136;
                                            } elseif (!$is137Disabled) {
                                                $defaultActiveId = 137;
                                            }

                                            // 3. Set initial values for display based on the selected variant
                                            if ($defaultActiveId == 135) {
                                                $initialStock = $stock135;
                                                $initialPrice = "3999";
                                            } elseif ($defaultActiveId == 136) {
                                                $initialStock = $stock136;
                                                $initialPrice = "4019";
                                            } elseif ($defaultActiveId == 137) {
                                                $initialStock = $stock137;
                                                $initialPrice = "4049";
                                            } else {
                                                $initialStock = 0;
                                                $initialPrice = "3999"; // Fallback price
                                            }

                                            $allOutOfStock = ($is135Disabled && $is136Disabled && $is137Disabled);
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 135) ? 'active' : ''; ?> <?php echo $is135Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="3999" 
                                                data-variant-id="135" 
                                                data-stock="<?php echo $stock135; ?>"
                                                <?php echo $is135Disabled ? 'disabled' : ''; ?>>
                                            Red <?php echo $is135Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 136) ? 'active' : ''; ?> <?php echo $is136Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="4019" 
                                                data-variant-id="136" 
                                                data-stock="<?php echo $stock136; ?>"
                                                <?php echo $is136Disabled ? 'disabled' : ''; ?>>
                                            Green <?php echo $is136Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 137) ? 'active' : ''; ?> <?php echo $is137Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="4049" 
                                                data-variant-id="137" 
                                                data-stock="<?php echo $stock137; ?>"
                                                <?php echo $is137Disabled ? 'disabled' : ''; ?>>
                                            Blue <?php echo $is137Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <form id="cartForm" action="buynow.php" method="POST" style="display:none;">
                                    ...
                                    <input type="hidden" id="variant_id" name="variant_id" value="<?php echo $defaultActiveId ?: 92; ?>">
