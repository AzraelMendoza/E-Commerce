  <?php
        ...
        $productId = 457; // 🔁 Change this per product page
        ...
        $initialStock = $variants[129]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/truck.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/truck.jpg" checked hidden>
                                    <img src="img/products/truck.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/truck2.jpg" hidden>
                                    <img src="img/products/truck2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/truck3.jpg" hidden>
                                    <img src="img/products/truck3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-7 ps-lg-5">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="d-flex gap-2">
                                    <?php 
                                        // 1. Fetch stock for variants 129, 130, and 131
                                        $stock129 = $variants[129]['stock_quantity'] ?? 0;
                                        $is129Disabled = ($stock129 <= 0);

                                        $stock130 = $variants[130]['stock_quantity'] ?? 0;
                                        $is130Disabled = ($stock130 <= 0);

                                        $stock131 = $variants[131]['stock_quantity'] ?? 0;
                                        $is131Disabled = ($stock131 <= 0);

                                        // 2. Logic: Auto-select the first available variant on load
                                        $defaultActiveId = null;
                                        if (!$is129Disabled) {
                                            $defaultActiveId = 129;
                                        } elseif (!$is130Disabled) {
                                            $defaultActiveId = 130;
                                        } elseif (!$is131Disabled) {
                                            $defaultActiveId = 131;
                                        }

                                        // 3. Set initial values for display based on the selected variant
                                        if ($defaultActiveId == 129) {
                                            $initialStock = $stock129;
                                            $initialPrice = "1449";
                                        } elseif ($defaultActiveId == 130) {
                                            $initialStock = $stock130;
                                            $initialPrice = "1549";
                                        } elseif ($defaultActiveId == 131) {
                                            $initialStock = $stock131;
                                            $initialPrice = "1569";
                                        } else {
                                            $initialStock = 0;
                                            $initialPrice = "1449"; // Default fallback
                                        }

                                        $allOutOfStock = ($is129Disabled && $is130Disabled && $is131Disabled);
                                    ?>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 129) ? 'active' : ''; ?> <?php echo $is129Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="1449" 
                                            data-variant-id="129" 
                                            data-stock="<?php echo $stock129; ?>"
                                            <?php echo $is129Disabled ? 'disabled' : ''; ?>>
                                        Construction <?php echo $is129Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 130) ? 'active' : ''; ?> <?php echo $is130Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="1549" 
                                            data-variant-id="130" 
                                            data-stock="<?php echo $stock130; ?>"
                                            <?php echo $is130Disabled ? 'disabled' : ''; ?>>
                                        Mining <?php echo $is130Disabled ? '(Sold Out)' : ''; ?>
                                    </button>

                                    <button type="button" 
                                            class="btn option-btn <?php echo ($defaultActiveId == 131) ? 'active' : ''; ?> <?php echo $is131Disabled ? 'disabled-variant' : ''; ?>" 
                                            data-price="1569" 
                                            data-variant-id="131" 
                                            data-stock="<?php echo $stock131; ?>"
                                            <?php echo $is131Disabled ? 'disabled' : ''; ?>>
                                        Dump <?php echo $is131Disabled ? '(Sold Out)' : ''; ?>
                                    </button>
                                </div>
                            </div>
