  <?php
        ...
        $productId = 456; // 🔁 Change this per product page
        ...
        $initialStock = $variants[127]['stock_quantity'] ?? 30;
                ...
                <div class="row g-5">
                    ...
                    <div class="col-lg-5">
                        <div class="sticky-top" style="top: 100px;">
                            <img id="mainImage" src="img/products/mix.jpg" class="product-img shadow-sm mb-3 w-100 rounded" alt="Product Image">

                            <div class="d-flex justify-content-center gap-3 mt-3">
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mix.jpg" checked hidden>
                                    <img src="img/products/mix.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mix2.jpg" hidden>
                                    <img src="img/products/mix2.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                                <label>
                                    <input type="radio" name="imgSelect" value="img/products/mixer3.jpg" hidden>
                                    <img src="img/products/mixer3.jpg" class="thumb" onclick="document.getElementById('mainImage').src=this.src">
                                </label>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="mt-3">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="d-flex gap-2">
                                        <?php 
                                            // 1. Fetch stock for variants 127 and 128
                                            $stock127 = $variants[127]['stock_quantity'] ?? 0;
                                            $is127Disabled = ($stock127 <= 0);

                                            $stock128 = $variants[128]['stock_quantity'] ?? 0;
                                            $is128Disabled = ($stock128 <= 0);

                                            // 2. Logic: Auto-select the first available color on load
                                            $defaultActiveId = null;
                                            if (!$is127Disabled) {
                                                $defaultActiveId = 127;
                                            } elseif (!$is128Disabled) {
                                                $defaultActiveId = 128;
                                            }

                                            // 3. Set initial display values based on the auto-selected variant
                                            if ($defaultActiveId == 127) {
                                                $initialStock = $stock127;
                                                $initialPrice = "1299";
                                            } elseif ($defaultActiveId == 128) {
                                                $initialStock = $stock128;
                                                $initialPrice = "1399";
                                            } else {
                                                $initialStock = 0;
                                                $initialPrice = "1299"; // Default fallback
                                            }

                                            $allOutOfStock = ($is127Disabled && $is128Disabled);
                                        ?>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 127) ? 'active' : ''; ?> <?php echo $is127Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="1299" 
                                                data-variant-id="127" 
                                                data-stock="<?php echo $stock127; ?>"
                                                <?php echo $is127Disabled ? 'disabled' : ''; ?>>
                                            Yellow <?php echo $is127Disabled ? '(Sold Out)' : ''; ?>
                                        </button>

                                        <button type="button" 
                                                class="btn option-btn <?php echo ($defaultActiveId == 128) ? 'active' : ''; ?> <?php echo $is128Disabled ? 'disabled-variant' : ''; ?>" 
                                                data-price="1399" 
                                                data-variant-id="128" 
                                                data-stock="<?php echo $stock128; ?>"
                                                <?php echo $is128Disabled ? 'disabled' : ''; ?>>
                                            Orange <?php echo $is128Disabled ? '(Sold Out)' : ''; ?>
                                        </button>
                                    </div>
                                </div>             
